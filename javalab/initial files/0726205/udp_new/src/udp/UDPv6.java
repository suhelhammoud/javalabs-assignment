...
*/
public class UDPv6 extends javax.swing.JFrame{
	...
	*/
	...
	private void TransferMouseClicked(MouseEvent evt) 
	{
		...
		//TODO add your code for Transfer.mouseClicked
	}
	...
	private void activateServer() throws Exception
	{
		...
		//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		  ...
		  System.out.println(":::= "+packetsizes+" Is it good?");
		  ...
		  System.out.println("Time out @"+SocketServer.getSoTimeout());
		  ...
		  System.out.println(":::= "+fileSize_long+" Is it okay?");
		  ...
		  ServerDisplay.append("\n ack0 reply sent (Packet size confirmed!)");
		  
		  reply="ack1";
		  Sync_data_ServerToClient=reply.getBytes();
		  //InetAddress ClientIPAddress =  receivePacket.getAddress();
		  //int ClientPort = receivePacket.getPort();
		  DatagramPacket sendPacket2 = new DatagramPacket(Sync_data_ServerToClient,Sync_data_ServerToClient.length,ClientIPAddress,ClientPort);
		  SocketServer.send(sendPacket2);
		  System.out.println("Replying ack 1....");
		  ServerDisplay.append("\n ack1 reply sent (Ready to accept file)");
		  //**********************End Reply***************************************
	...
	private void BeginTransfer() throws Exception
	{
		...
		long value;
		long currentSize;
		size=10; //range should be always 10
		...
		byte[] ClientToSend = new byte[10];  //packet size for packet synchronisation
		...
		//end new code
			...
			//***********************************
					...
					//ClientSendPacket.
						...
						clientSockets.send(ClientSendPacket1);
						...
						DatagramPacket receivePacket2 = new DatagramPacket(AckResponseFromServer, AckResponseFromServer.length);
						clientSockets.receive(receivePacket2);
						String displayFromServer2 = new String(receivePacket2.getData());
						ClientArea.append("\n"+displayFromServer2);
						
						
						//*** latest ***//
						ClientArea.append("Synchronized! Sending "+TransmitFile.getPath());
						...
						try{
							...
							ClientArea.append("Total packets sent:"+clientNumberPacketsx);
							fin.close();
